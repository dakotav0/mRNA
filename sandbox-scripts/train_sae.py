...
def train(
    ...
    for name, w in zip(concepts, class_weights.tolist()):
        print(f"  {name:>14}: {w/max_w:.3f}  ({concept_counts[concepts.index(name)]} examples)")
    ...
    for epoch in range(1, epochs + 1):
        ...
        n = len(train_loader)
        train_loss /= n; train_mse /= n; train_l1 /= n; train_cb /= n
...
def main():
    ...
    parser.add_argument(
        "--activations", nargs="+", metavar="NAME:FILE", required=True,
    ...
    parser.add_argument("--l1-coeff",         type=float, default=0.01,
                        help="Weight on L1 sparsity penalty")
    parser.add_argument("--cb-coeff",         type=float, default=1.0,
