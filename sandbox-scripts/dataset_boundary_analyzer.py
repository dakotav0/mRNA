...
def main():
    parser = argparse.ArgumentParser(description="Analyze boundary overlap of a candidate dataset using the current SAE.")
    ...
    parser.add_argument("--text-column", default="instruction", help="Fallback primary text column (instruction, question, etc.)")
    parser.add_argument("--text-column2", default="output", help="Fallback secondary text column (e.g. output, answer, message_2)")
    parser.add_argument("--max-examples", type=int, default=200, help="Number of examples to profile")
    ...
    parser.add_argument("--layer", type=int, default=25, help="Layer to extract activations from")
    parser.add_argument("--d-model", type=int, default=1536)
    parser.add_argument("--max-seq-len", type=int, default=512)
    parser.add_argument("--concepts", default="biology,chemistry,physics", help="Comma-separated concepts in the trained SAE")
    ...
    if format_success == 0:
        print("[ERROR] Could not extract any valid text from the dataset. Check column names.")
        sys.exit(1)

    print(f"Format Match: ✅ successfully extracted text from {format_success}/{max_examples} examples.")
    ...
    if not os.path.exists(sae_weights):
        print(f"[ERROR] Trained SAE weights not found at {sae_weights}. Cannot analyze bleed without a trained SAE.")
        sys.exit(1)
    ...
    sae = CBSAE(d_model=args.d_model, expansion_factor=saved_expansion, bottleneck_features=n_concepts)
    ...
    for i, name in enumerate(concept_names):
        print(f"  {name.capitalize()}: {avg_confidences[i].item():.4f} (Max Peak: {max_confidences[i].item():.4f})")
    
    print("\n[Analysis]")
    print(f"Highest Average Bleed: {highest_bleed_concept.capitalize()} ({highest_bleed_score:.4f})")
    ...
    if highest_bleed_score > 0.7:
        ...
        print("Recommendation: Filter or select an alternative dataset to avoid catastrophic interference.")
        ...
        print("This dataset has noticeable similarity to existing concepts but is not entirely dominant.")
        print("Recommendation: Proceed with caution or increase sample size for better certainty.")
