...
def socket_block_sparse_kernel(
    ...
    q_ptrs = q_hash_ptr + (
        rm[:, None] * stride_qh_seq + r_hash[None, :] * stride_qh_hash
    )
    k_ptrs = k_hash_ptr + (
        rn[:, None] * stride_kh_seq + r_hash[None, :] * stride_kh_hash
    )
...
def run_socket_triton_sim(seq_len=4096, hash_dim=32, threshold=0.75):
    ...
    q_hashes = torch.randint(
        0, 2, (seq_len, hash_dim), device="cuda", dtype=torch.float32
    )
    k_hashes = torch.randint(
        0, 2, (seq_len, hash_dim), device="cuda", dtype=torch.float32
    )
    ...
    print(
        f"Launching SOCKET Triton Kernel on grid {triton.cdiv(seq_len, BLOCK_M)}x{triton.cdiv(seq_len, BLOCK_N)}..."
    )
    ...
    print(
        f"Triton Kernel Finished! Active connections permitted: {active_blocks:,} / {seq_len * seq_len:,}"
    )
    print(
        "Crucially, we never allocated a massive float32 FP32 dense attention matrix in PyTorch."
    )
