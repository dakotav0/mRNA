...
class EngramHostTable:
    ...
    def __init__(self, table_size=1000000, embedding_dim=2048):
        ...
        print(
            f"Allocating {table_size} Engram entries of size {embedding_dim} in CPU RAM..."
        )
        ...
        self.memory_bank = torch.zeros(
            (table_size, embedding_dim), dtype=torch.float16
        ).pin_memory()
        ...
        print(
            f"RAM Cost: {self.memory_bank.element_size() * self.memory_bank.nelement() / (1024**3):.2f} GB"
        )
    ...
    def async_prefetch_to_gpu(
        self, ngram_tokens: list[int], gpu_stream: torch.cuda.Stream
    ) -> torch.Tensor:
        """
...
if __name__ == "__main__":
    ...
    dummy_work = torch.matmul(
        torch.randn(4096, 4096, device="cuda"), torch.randn(4096, 4096, device="cuda")
    )
    ...
    print(
        f"Success! Payload arrived on GPU. Shape: {gpu_factual_payload.shape}, Device: {gpu_factual_payload.device}"
    )
