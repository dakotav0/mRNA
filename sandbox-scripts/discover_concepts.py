...
def main():
    parser = argparse.ArgumentParser(
        description="Unsupervised Concept Discovery via HDBSCAN and LLM auto-labeling."
    )
    parser.add_argument(
        "--dataset",
        required=True,
    ...
    parser.add_argument(
        "--text-column", default="instruction", help="Fallback primary text column"
    )
    parser.add_argument(
        "--text-column2", default="output", help="Fallback secondary text column"
    )
    ...
    parser.add_argument(
        "--layer", type=int, default=25, help="Transformer layer index to intercept"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5000,
        help="Number of examples to profile broadly",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="Holdout split for approved clusters",
    )
    ...
    print(f"\n{'=' * 60}")
    print("mRNA Unsupervised Concept Discovery")
    print(f"{'=' * 60}")
    ...
    print(
        f"Extracting up to {args.max_examples} latent vectors (prefetch buffer: {args.prefetch_buffer})..."
    )
    while True:
        txt = prefetch_q.get()
        if txt is None:
    ...
    if n_clusters == 0:
        print(
            "[WARNING] HDBSCAN failed to find distinct clusters. The dataset might be too uniform or too small."
        )
        sys.exit(0)
    ...
    print(
        "Review discovered concepts. You may [A]ccept, [R]ename, or [D]iscard them.\n"
    )
    ...
    for label_id, meta in cluster_metadata.items():
        ...
        for i, s in enumerate(
            meta["samples"][:2]
        ):  # Just show 2 samples to keep terminal clean
            snip = (
                s[:150].replace("\n", " ") + "..."
                if len(s) > 150
                else s.replace("\n", " ")
        ...
        while True:
            choice = (
                input(
                    f"\nAction for '{meta['name']}' ([A]ccept / [R]ename / [D]iscard): "
                )
                .strip()
                .lower()
            )
