...
def evaluate_holdout(file_path: str, sae: CBSAE, concept_idx: int, n_concepts: int) -> dict:
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate SAE routing matrix on holdout datasets.")
    parser.add_argument("--model-id", default="unsloth/gemma-4-E2B-it", help="Model ID or name to evaluate")
    ...
    sae = CBSAE(d_model=args.d_model, expansion_factor=8, bottleneck_features=n_concepts)
    ...
    holdout_files = glob.glob(os.path.join(holdout_dir, "**", "*_test.pt"), recursive=True)
            
    if not holdout_files:
        print(f"No valid _test.pt holdout files found in {holdout_dir}. Ensure you have harvested data with --holdout-ratio > 0.")
        sys.exit(0)
    ...
    for file_path in holdout_files:
        ...
        if mapped_concept is None:
            print(f"[WARNING] Skipping {filename} - could not infer concept from {concept_list}")
            continue
    ...
    for concept in concept_list:
        if concept in results_map:
            ...
            print(f"{concept.capitalize():<15} | {acc_str:<8} | {metrics['total']:<6} | {bleed_str}")
        else:
            print(f"{concept.capitalize():<15} | {'N/A':<8} | {'-':<6} | No holdout data evaluated")

    final_acc = (overall_correct / overall_total) * 100 if overall_total > 0 else 0
    print("-" * 70)
    print(f"OVERALL PERFORMANCE: {final_acc:.2f}% ({overall_correct}/{overall_total} successful routes)")
    print("="*70 + "\n")
