...
def run_e2e_pipeline(
    ...
    for concept in concepts:
        ...
        path = MRNAPaths.DATA / mid / "activations" / f"layer_{layer}" / f"{concept}_train.pt"
        activation_files.append(str(path))

    # 2. SAE Training
    print("\n--- Training SAE weights ---")
    train_sae_weights(concepts=concepts, activation_files=activation_files, epochs=sae_epochs)
    ...
    if run_eval:
        ...
        print(f"Pipeline complete. Global Accuracy: {results.get('overall_accuracy', 0):.2%}")

def main():
    parser = argparse.ArgumentParser(description=".mrna Auto-Train Pipeline")
    parser.add_argument("--concepts", help="Comma-separated concepts (e.g. biology,math)")
    parser.add_argument("--model", help="Model ID override")
    parser.add_argument("--examples", type=int, default=5000, help="Examples to harvest per concept")
    ...
    concept_list = args.concepts.split(",") if args.concepts else list(config.science_triad_datasets.keys())
