...
def get_dataset_formatter(
    dataset_id: str, tokenizer, text_column: Optional[str] = None
):
    ...
    if text_column:
        ...
        return lambda ex: {
            "text": [
                CHAT_PROMPT.format(
                    topic=ex.get("topic", [""])[i] or "science", question=q, answer=a
                )
                + tokenizer.eos_token
        ...
        return lambda ex: {
            "text": [
                ALPACA_PROMPT.format(instruction=i, input=inp, output=o)
                + tokenizer.eos_token
...
def train_adapter(
    ...
    **kwargs,
    ...
    output_dir = kwargs.get(
        "output_dir", str(MRNAPaths.ROOT / "adapters" / f"{concept}_lora")
    )

    # 1. Load Model
    backend = get_backend("torch")
    model, tokenizer = backend.load_model(
        mid, max_seq_length=max_seq_length, load_in_4bit=True
    )
    ...
    rows = list(
        tqdm(ds_stream.take(kwargs.get("max_examples", 5000)), desc="Streaming")
    )
