...
class StreamingExecutionNode:
    ...
    """
    def __init__(self, model_id="unsloth/Llama-3.2-1B-Instruct", max_vram=0.6, max_loras=2):
    ...
    def _build_gemma4_text_cache(src: str, cache_dir: str) -> None:
        ...
        print("[StreamingExecutionNode] Building Gemma4 text-only vLLM cache "
        ...
        for shard_file in shards:
            ...
            print(f"  Loaded {len(text_tensors)} text tensors so far (shard: {shard_file})")
    ...
    def run_inference_with_adapter(self, prompt: str, adapter_path: str, adapter_id: int):
        """
        ...
        if not os.path.exists(adapter_path):
            print(f"[Execution Node] NOTE: '{adapter_path}' does not exist on disk yet.")
