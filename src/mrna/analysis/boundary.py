...
def analyze_dataset_boundary(
    ...
    **kwargs,
    ...
    ds = load_dataset(
        dataset_id, split=kwargs.get("split", "train"), streaming=True
    ).take(max_examples)
    batch_texts = []
    fallback_cols = [
        "text",
    ...
    for ex in ds:
        txt = extract_text2(
            ex, kwargs.get("text_column", "instruction"), None, fallback_cols
        )
        if txt.strip():
    ...
    with torch.no_grad():
        for i in range(0, len(batch_texts), batch_size):
            ...
            for act in interceptor.intercepted_activations:
                pooled_activations.append(
                    masked_mean_pool(act, enc["attention_mask"].cpu())
                )
    ...
    sae = CBSAE(
        d_model=d_model, expansion_factor=expansion, bottleneck_features=len(concepts)
    )
    ...
    report = {
        "dataset": dataset_id,
        "avg_confidences": {
            name: avg_probs[i].item() for i, name in enumerate(concepts)
        },
...
def main():
    ...
    report = analyze_dataset_boundary(
        dataset_id=args.dataset, concepts=args.concepts.split(",")
    )
    print(report)
